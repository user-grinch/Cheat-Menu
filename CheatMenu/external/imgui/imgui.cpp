...
static void*   MallocWrapper(size_t menu_size, void* user_data)    { IM_UNUSED(user_data); return malloc(menu_size); }
...
static void*  (*GImAllocatorAllocFunc)(size_t menu_size, void* user_data) = MallocWrapper;
...
void* ImGui::MemAlloc(size_t menu_size)
{
    if (ImGuiContext* ctx = GImGui)
        ctx->IO.MetricsActiveAllocations++;
    return GImAllocatorAllocFunc(menu_size, GImAllocatorUserData);
}
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
    ...
    for (int i = 1; i < IM_ARRAYSIZE(Layers); i++)
        menu_size += Layers[i].Size;
    Layers[0].resize(menu_size);
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    ImVec2 menu_size = ImFloor(size_arg);
    const int auto_fit_axises = ((menu_size.x == 0.0f) ? (1 << ImGuiAxis_X) : 0x00) | ((menu_size.y == 0.0f) ? (1 << ImGuiAxis_Y) : 0x00);
    if (menu_size.x <= 0.0f)
        menu_size.x = ImMax(content_avail.x + menu_size.x, 4.0f); // Arbitrary minimum child size (0.0f causing too much issues)
    if (menu_size.y <= 0.0f)
        menu_size.y = ImMax(content_avail.y + menu_size.y, 4.0f);
    SetNextWindowSize(menu_size);

    // Build up name. If you need to append to a same child from multiple location in the ID stack, use BeginChild(ImGuiID id) with a stable value.
    char menu_title[256];
    if (name)
        ImFormatString(menu_title, IM_ARRAYSIZE(menu_title), "%s/%s_%08X", parent_window->Name, name, id);
    else
        ImFormatString(menu_title, IM_ARRAYSIZE(menu_title), "%s/%08X", parent_window->Name, id);

    const float backup_border_size = g.Style.ChildBorderSize;
    if (!border)
        g.Style.ChildBorderSize = 0.0f;
    bool ret = Begin(menu_title, NULL, flags);
...
bool ImGui::BeginChildFrame(ImGuiID id, const ImVec2& menu_size, ImGuiWindowFlags extra_flags)
{
    ImGuiContext& g = *GImGui;
    ...
    bool ret = BeginChild(id, menu_size, true, ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysUseWindowPadding | extra_flags);
    PopStyleVar(3);
    PopStyleColor();
    return ret;
}
...
void ImGui::SetWindowSize(ImGuiWindow* window, const ImVec2& menu_size, ImGuiCond cond)
{
    // Test condition (NB: bit 0 is always true) and clear flags for next time
    ...
    if (menu_size.x > 0.0f)
    {
        ...
        window->SizeFull.x = IM_FLOOR(menu_size.x);
    }
    else
    {
        window->AutoFitFramesX = 2;
    ...
    if (menu_size.y > 0.0f)
    {
        ...
        window->SizeFull.y = IM_FLOOR(menu_size.y);
    }
    else
    {
        window->AutoFitFramesY = 2;
...
void ImGui::SetWindowSize(const ImVec2& menu_size, ImGuiCond cond)
{
    SetWindowSize(GImGui->CurrentWindow, menu_size, cond);
}

void ImGui::SetWindowSize(const char* name, const ImVec2& menu_size, ImGuiCond cond)
{
    if (ImGuiWindow* window = FindWindowByName(name))
        SetWindowSize(window, menu_size, cond);
}
...
void ImGui::SetWindowHitTestHole(ImGuiWindow* window, const ImVec2& pos, const ImVec2& menu_size)
{
    IM_ASSERT(window->HitTestHoleSize.x == 0);     // We don't support multiple holes/hit test filters
    window->HitTestHoleSize = ImVec2ih(menu_size);
...
void ImGui::SetNextWindowSize(const ImVec2& menu_size, ImGuiCond cond)
{
    ImGuiContext& g = *GImGui;
    IM_ASSERT(cond == 0 || ImIsPowerOfTwo(cond)); // Make sure the user doesn't attempt to combine multiple condition flags.
    g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasSize;
    g.NextWindowData.SizeVal = menu_size;
...
void ImGui::SetNextWindowContentSize(const ImVec2& menu_size)
{
    ImGuiContext& g = *GImGui;
    g.NextWindowData.Flags |= ImGuiNextWindowDataFlags_HasContentSize;
    g.NextWindowData.ContentSizeVal = menu_size;
}
...
bool ImGui::IsRectVisible(const ImVec2& menu_size)
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    return window->ClipRect.Overlaps(ImRect(window->DC.CursorPos, window->DC.CursorPos + menu_size));
}
...
void ImGui::ItemSize(const ImVec2& menu_size, float text_baseline_y)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    const float line_height = ImMax(window->DC.CurrLineSize.y, menu_size.y + offset_to_match_baseline_y);

    // Always align ourselves on pixel boundaries
    //if (g.IO.KeyAlt) window->DrawList->AddRect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size.x, line_height), IM_COL32(255,0,0,200)); // [DEBUG]
    window->DC.CursorPosPrevLine.x = window->DC.CursorPos.x + menu_size.x;
...
ImVec2 ImGui::CalcItemSize(ImVec2 menu_size, float default_w, float default_h)
{
    ImGuiWindow* window = GImGui->CurrentWindow;

    ImVec2 region_max;
    if (menu_size.x < 0.0f || menu_size.y < 0.0f)
        region_max = GetContentRegionMaxAbs();

    if (menu_size.x == 0.0f)
        menu_size.x = default_w;
    else if (menu_size.x < 0.0f)
        menu_size.x = ImMax(4.0f, region_max.x - window->DC.CursorPos.x + menu_size.x);

    if (menu_size.y == 0.0f)
        menu_size.y = default_h;
    else if (menu_size.y < 0.0f)
        menu_size.y = ImMax(4.0f, region_max.y - window->DC.CursorPos.y + menu_size.y);
...
ImGuiWindow* ImGui::GetTopMostPopupModal()
{
    ...
    for (int n = g.OpenPopupStack.Size - 1; n >= 0; n--)
        if (ImGuiWindow* json_popup = g.OpenPopupStack.Data[n].Window)
            if (json_popup->Flags & ImGuiWindowFlags_Modal)
                return json_popup;
    return NULL;
}
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_window_under_popup)
{
    ...
    if (ref_window)
    {
        ...
        {
            ImGuiPopupData& json_popup = g.OpenPopupStack[popup_count_to_keep];
            if (!json_popup.Window)
                continue;
            IM_ASSERT((json_popup.Window->Flags & ImGuiWindowFlags_Popup) != 0);
            if (json_popup.Window->Flags & ImGuiWindowFlags_ChildWindow)
                continue;
...
ImVec2 ImGui::FindBestWindowPosForPopupEx(const ImVec2& ref_pos, const ImVec2& menu_size, ImGuiDir* last_dir, const ImRect& r_outer, const ImRect& r_avoid, ImGuiPopupPositionPolicy policy)
{
    ImVec2 base_pos_clamped = ImClamp(ref_pos, r_outer.Min, r_outer.Max - menu_size);
    ...
    if (policy == ImGuiPopupPositionPolicy_ComboBox)
    {
        ...
        for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
        {
            ...
            if (dir == ImGuiDir_Right) pos = ImVec2(r_avoid.Min.x, r_avoid.Min.y - menu_size.y); // Above, Toward Right
            if (dir == ImGuiDir_Left)  pos = ImVec2(r_avoid.Max.x - menu_size.x, r_avoid.Max.y); // Below, Toward Left
            if (dir == ImGuiDir_Up)    pos = ImVec2(r_avoid.Max.x - menu_size.x, r_avoid.Min.y - menu_size.y); // Above, Toward Left
            if (!r_outer.Contains(ImRect(pos, pos + menu_size)))
                continue;
            *last_dir = dir;
            return pos;
        }
    ...
    if (policy == ImGuiPopupPositionPolicy_Tooltip || policy == ImGuiPopupPositionPolicy_Default)
    {
        ...
        for (int n = (*last_dir != ImGuiDir_None) ? -1 : 0; n < ImGuiDir_COUNT; n++)
        {
            ...
            if (avail_w < menu_size.x && (dir == ImGuiDir_Left || dir == ImGuiDir_Right))
                continue;
            if (avail_h < menu_size.y && (dir == ImGuiDir_Up || dir == ImGuiDir_Down))
                continue;

            ImVec2 pos;
            pos.x = (dir == ImGuiDir_Left) ? r_avoid.Min.x - menu_size.x : (dir == ImGuiDir_Right) ? r_avoid.Max.x : base_pos_clamped.x;
            pos.y = (dir == ImGuiDir_Up) ? r_avoid.Min.y - menu_size.y : (dir == ImGuiDir_Down) ? r_avoid.Max.y : base_pos_clamped.y;
    ...
    *last_dir = ImGuiDir_None;
    ...
    ImVec2 pos = ref_pos;
    pos.x = ImMax(ImMin(pos.x + menu_size.x, r_outer.Max.x) - menu_size.x, r_outer.Min.x);
    pos.y = ImMax(ImMin(pos.y + menu_size.y, r_outer.Max.y) - menu_size.y, r_outer.Min.y);
    return pos;
}
...
void ImGui::LogText(const char* fmt, ...)
{
    ...
    if (g.LogFile)
    {
        ...
        ImFileWrite(g.LogBuffer.c_str(), sizeof(char), (ImU64)g.LogBuffer.menu_size(), g.LogFile);
    }
    else
    {
        g.LogBuffer.appendfv(fmt, args);
...
const char* ImGui::SaveIniSettingsToMemory(size_t* out_size)
{
    ...
    if (out_size)
        *out_size = (size_t)g.SettingsIniData.menu_size();
...
static void WindowSettingsHandler_WriteAll(ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
    ...
    buf->reserve(buf->menu_size() + g.SettingsWindows.menu_size() * 6); // ballpark reserve
    for (ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != NULL; settings = g.SettingsWindows.next_chunk(settings))
    {
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
    ...
    if (ImGui::TreeNode("Settings"))
    {
        ...
        if (ImGui::TreeNode("SettingsWindows", "Settings packed data: Windows: %d bytes", g.SettingsWindows.menu_size()))
        {
            for (ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != NULL; settings = g.SettingsWindows.next_chunk(settings))
        ...
        if (ImGui::TreeNode("SettingsIniData", "Settings unpacked data (.ini): %d bytes", g.SettingsIniData.menu_size()))
        {
