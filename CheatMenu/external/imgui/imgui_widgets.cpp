...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 menu_size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);

    const ImRect bb(pos, pos + menu_size);
    ItemSize(menu_size, style.FramePadding.y);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 menu_size = CalcItemSize(size_arg, 0.0f, 0.0f);
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menu_size);
    ItemSize(menu_size);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 menu_size, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ItemSize(menu_size, (menu_size.y >= default_size) ? g.Style.FramePadding.y : -1.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    RenderArrow(window->DrawList, bb.Min + ImVec2(ImMax(0.0f, (menu_size.x - g.FontSize) * 0.5f), ImMax(0.0f, (menu_size.y - g.FontSize) * 0.5f)), text_col, dir);
...
void ImGui::Image(ImTextureID user_texture_id, const ImVec2& menu_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& tint_col, const ImVec4& border_col)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;

    ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menu_size);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureID texture_id, const ImVec2& menu_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec2& padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menu_size + padding * 2);
    ItemSize(bb);
    if (!ItemAdd(bb, id))
        return false;
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& menu_size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    return ImageButtonEx(id, user_texture_id, menu_size, uv0, uv1, padding, bg_col, tint_col);
}
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 menu_size = CalcItemSize(size_arg, CalcItemWidth(), g.FontSize + style.FramePadding.y * 2.0f);
    ImRect bb(pos, pos + menu_size);
    ItemSize(menu_size, style.FramePadding.y);
    if (!ItemAdd(bb, 0))
        return;
...
void ImGui::Dummy(const ImVec2& menu_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
...
bool ImGui::VSliderScalar(const char* label, const ImVec2& menu_size, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + menu_size);
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& menu_size, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    return VSliderScalar(label, menu_size, ImGuiDataType_Float, v, &v_min, &v_max, format, flags);
}

bool ImGui::VSliderInt(const char* label, const ImVec2& menu_size, int* v, int v_min, int v_max, const char* format, ImGuiSliderFlags flags)
{
    return VSliderScalar(label, menu_size, ImGuiDataType_S32, v, &v_min, &v_max, format, flags);
}
...
bool ImGui::InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& menu_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    return InputTextEx(label, NULL, buf, (int)buf_size, menu_size, flags | ImGuiInputTextFlags_Multiline, callback, user_data);
}
...
static void    STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, STB_TEXTEDIT_STRING* obj, int line_start_idx)
{
    ...
    const ImVec2 menu_size = InputTextCalcTextSizeW(text + line_start_idx, text + obj->CurLenW, &text_remaining, NULL, true);
    r->x0 = 0.0f;
    r->x1 = menu_size.x;
    r->baseline_y_delta = menu_size.y;
    r->ymin = 0.0f;
    r->ymax = menu_size.y;
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 menu_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (menu_size.x == 0.0f)
        menu_size.x = default_size;
    if (menu_size.y == 0.0f)
        menu_size.y = default_size;
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + menu_size);
    ItemSize(bb, (menu_size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    float grid_step = ImMin(menu_size.x, menu_size.y) / 2.99f;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 menu_size(size_arg.x != 0.0f ? size_arg.x : label_size.x, size_arg.y != 0.0f ? size_arg.y : label_size.y);
    ImVec2 pos = window->DC.CursorPos;
    pos.y += window->DC.CurrLineTextBaseOffset;
    ItemSize(menu_size, 0.0f);
    ...
    if (size_arg.x == 0.0f || (flags & ImGuiSelectableFlags_SpanAvailWidth))
        menu_size.x = ImMax(label_size.x, max_x - min_x);

    // Text stays at the submission position, but bounding box may be extended on both sides
    const ImVec2 text_min = pos;
    const ImVec2 text_max(min_x + menu_size.x, pos.y + menu_size.y);
...
bool ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 menu_size = CalcItemSize(size_arg, CalcItemWidth(), GetTextLineHeightWithSpacing() * 7.4f + style.ItemSpacing.y);
    ImVec2 frame_size = ImVec2(menu_size.x, ImMax(menu_size.y, label_size.y));
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    ImVec2 menu_size;
    menu_size.x = 0.0f;
    menu_size.y = ImFloor(GetTextLineHeightWithSpacing() * height_in_items_f + style.FramePadding.y * 2.0f);
    return ListBoxHeader(label, menu_size);
}
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    ImVec2 menu_size = TabItemCalcSize(label, p_open != NULL);
    ...
    if (tab == NULL)
    {
        ...
        tab->ID = id;
        tab->Width = menu_size.x;
        tab_bar->TabsAddedNew = true;
        tab_is_new = true;
    }
    tab_bar->LastTabItemIdx = (short)tab_bar->Tabs.index_from_ptr(tab);
    tab->ContentWidth = menu_size.x;
    ...
    tab->NameOffset = (ImS16)tab_bar->TabsNames.menu_size();
...
ImVec2 ImGui::TabItemCalcSize(const char* label, bool has_close_button)
{
    ImGuiContext& g = *GImGui;
    ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 menu_size = ImVec2(label_size.x + g.Style.FramePadding.x, label_size.y + g.Style.FramePadding.y * 2.0f);
    if (has_close_button)
        menu_size.x += g.Style.FramePadding.x + (g.Style.ItemInnerSpacing.x + g.FontSize); // We use Y intentionally to fit the close button circle.
    else
        menu_size.x += g.Style.FramePadding.x + 1.0f;
    return ImVec2(ImMin(menu_size.x, TabBarCalcMaxTabWidth()), menu_size.y);
}
